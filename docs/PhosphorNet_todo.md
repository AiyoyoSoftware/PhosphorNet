...
| Action daemon service packaging | `phosphor-actiond` is implemented but not yet shipped with init-system units or automatic service-account/group creation. | The binary, strict TOML rules, Unix-socket JSON protocol, two-layer door authorization, bounded execution, callbacks, audit records, installer, and release artifacts exist. Operators currently supervise both daemons and configure socket ownership themselves. | `docs/PhosphorNet_configuration.md`, `internal/action/`, `internal/node/doors.go` |
