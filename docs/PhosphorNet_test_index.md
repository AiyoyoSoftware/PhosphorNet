...
| Host action delegation | `internal/action/action_test.go`: rule parsing, door denial, timeout/output caps, and Unix-socket JSON round trip; `internal/runtime/invoke_test.go`: Lua action request/result helpers and fixed-rule bundled Action Workshop behavior; `internal/node/integration_test.go`: `TestIntegrationDoorActionRoundTripReturnsOutputToDoor`; `internal/node/capabilities_test.go`: rule-specific capability enforcement | Covered |
